...
(defn valid-user? [username]
  (vali/rule (not (db/get-in :users [username]))
             [:username "That username is already taken, vec postoji to ime"])
  (vali/rule (vali/min-length? username 3)
             [:username "Username must be at least 3 characters. Ime mora najmanje sadrzati 3 slova"])
  (not (vali/errors? :username :password)))

(defn valid-psw? [password]
  (vali/rule (vali/min-length? password 5)
             [:password "Password must be at least 5 characters. Lozinka mora imati najmanje 5 slova"])
...
(defn login! [{:keys [username password] :as user}]
  (let [{stored-pass :password} (get-username username)]
    (if (and stored-pass 
      ...
      (vali/set-error :username "Invalid username or password, Nepravilno ime ili lozinka"))))
